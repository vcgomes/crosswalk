...
bool GeneratePkgInfoXml(xwalk::application::ApplicationData* application,
  ...
  base::FilePath dir_xml(xml_path.DirName());
  if (!base::PathExists(dir_xml) &&
      !file_util::CreateDirectory(dir_xml))
    return false;

  std::string package_id = application->ID();
  base::FilePath execute_path = app_dir.AppendASCII("bin/").AppendASCII(package_id);
...

  {
    ...
    if (icon_name.empty())
      ...
      xml_writer.WriteElement("icon",
                              std::string(package_id + ".png").MaybeAsASCII());
...
bool InstallApplicationForTizen(
		ApplicationData* application, base::FilePath& data_dir)
{
  base::string package_id = application->ID();
  ...
  if (!GeneratePkgInfoXml(application, icon_name, xml_path)) {
    return false;
  }

  base::FilePath app_dir = data_dir.AppendASCII(info::kAppDir).AppendASCII(package_id);
  base::FilePath execute_path = app_dir.AppendASCII("bin/").AppendASCII(package_id);
  ...
  base::FilePath icon = app_dir.AppendASCII("src/").AppendASCII(icon_name);
